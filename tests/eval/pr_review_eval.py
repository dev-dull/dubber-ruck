...
TRAPS = {
    "rand.Seed demanded": re.compile(r"rand\.Seed", re.I),
    # A fall is an *assertion* that the sleep blocks. "without blocking", "does not
    # block", "non-blocking" are the correct answer and must not count.
    "sleep blocks UI": re.compile(
        r"sleep.{0,120}?(?<!without )(?<!not )(?<!never )(?<!non-)(?<!no )(block|freez|hang|stall)\w*.{0,80}(ui|terminal|input|event loop|render)",
        re.I | re.S,
    ),
}
...
def _selfcheck():
    rx = TRAPS["sleep blocks UI"]
    assert rx.search("the sleep blocks the terminal")
    assert rx.search("time.Sleep will freeze input processing")
    assert not rx.search("the sleep throttles the rate without blocking the UI event loop")
    assert not rx.search("time.Sleep does not block the event loop")
